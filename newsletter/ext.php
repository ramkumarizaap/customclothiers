			...
			if($member_id){
				...
				if(isset($newsletter->settings['double_opt_in']) && strtolower($newsletter->settings['double_opt_in']) == 'yes'){
					...
					$replace = array(
						...
						"CONFIRM_SUBSCRIPTION"=>'https://'.$newsletter->base_href.'/ext.php?t=confirm&mid='.$member_id.'&hash='.md5("double".$member_id).'',
	...
	case "view":
		...
		if(($mhash_real == $mhash_provided) || ($provided_hash == $real_hash)){ 
			...
			$replace = array(
				...
				"unsubscribe_url" => 'https://'.$newsletter->base_href.'/ext.php?t=unsub',

				"view_online"=>'https://'.$newsletter->base_href.'/ext.php?t=view&id='.$send_data['newsletter_id'].'&sid='.$send_id.'&mid='.$member_data['member_id'].'&hash='.md5("view link ".$member_data['member_id']."from $send_id").'',
			...
			$replace = array(
				...
				"unsubscribe_url" => 'https://'.$newsletter->base_href.'/ext.php?t=unsub_form',

				"view_online"=>'https://'.$newsletter->base_href.'/ext.php?t=view&id='.$id,

				"link_account" => 'https://'.$newsletter->base_href.'/ext.php?t=update_form',
