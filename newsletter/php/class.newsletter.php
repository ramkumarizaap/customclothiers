...
class newsletter{
				...
				foreach($links[2] as $link_id => $l){
					...
					if(!preg_match('/^\{/',$l) && !preg_match('/^#/',$l) && !(preg_match('/^\w+:/',$l) && !preg_match('/^http/',$l))){
						...
						if($send_id){
							...
							$sql = "INSERT INTO `"._DB_PREFIX."link` SET link_url = '".mysqli_real_escape_string($newlink)."'";

							$res = query($sql,$db);

							$link_id = mysqli_insert_id($db);

							$newlink = 'https://'.$this->base_href.'/ext.php?t=lnk&id='.$link_id.'&sid={SEND_ID}&mid={MEMBER_ID}&mhash={MEMBER_HASH}';
					...
					if(!preg_match('/^\{/',$l) && !preg_match('/^#/',$l) && !(preg_match('/^\w+:/',$l) && !preg_match('/^http/',$l))){
						...
						if(!isset($this->settings['inline_images']) || $this->settings['inline_images'] == 'no' || !$this->settings['inline_images']){
							...
							if($send_id){
								...
								$sql = "INSERT INTO `"._DB_PREFIX."image` SET image_url = '".mysqli_real_escape_string($newlink)."'";

								$res = query($sql,$db);

								$link_id = mysqli_insert_id($db);

								$newlink = 'https://'.$this->base_href.'/ext.php?t=img&id='.$link_id.'&sid={SEND_ID}&mid={MEMBER_ID}';
	...
	public function save($db=false,$newsletter_id='new',$fields){
		...
		if($newsletter_id == 'new'){
			...
			$where = " WHERE newsletter_id = '".$newsletter_id."' LIMIT 1";
	...
	public function save_content($db=false,$newsletter_id,$newsletter_content_id){
		...
		if($newsletter_content_id == 'new'){
			...
			$where = " WHERE newsletter_id = '$newsletter_id' AND newsletter_content_id = '".mysqli_real_escape_string($newsletter_content_id)."' LIMIT 1";
		...
		foreach($fields as $key=>$val){
			...
			$sql .= ", `".$key."` = '".mysqli_real_escape_string($val)."'";
		...
		if($newsletter_content_id=='new'){
			...
			$newsletter_content_id = mysqli_insert_id($db);
	...
	public function write_newsletter_content($db,$newsletter_id){
		...
		$replace = array(
			...
			"unsubscribe_url" => 'https://'.$this->base_href.'/ext.php?t=unsub',

			"view_online"=>'https://'.$this->base_href.'/ext.php?t=view&id='.$newsletter_id.'',

			"SENDTOFRIEND"=>'https://'.$this->base_href.'/ext.php?t=send_to_friend&id='.$newsletter_id.'',

			"link_account"=>'https://'.$this->base_href.'/ext.php?t=update_form',
	...
	public function delete_newsletter($db,$newsletter_id){
		...
		$sql = "DELETE FROM "._DB_PREFIX."newsletter WHERE newsletter_id = '".mysqli_real_escape_string($newsletter_id)."'";
	...
	public function get_newsletter($db=false,$newsletter_id){
		...
		$sql = "SELECT * FROM "._DB_PREFIX."newsletter WHERE newsletter_id = '".mysqli_real_escape_string($newsletter_id)."'";
	...
	public function get_group($db,$group_id){
		...
		$sql = "SELECT * FROM `"._DB_PREFIX."group` WHERE `group_id` = '".$group_id."'";
	...
	public function save_group($db=false,$group_id='new',$group_name,$public=0){
		...
		if($group_id == 'new'){
			...
			$sql = "INSERT INTO `"._DB_PREFIX."group` SET group_name = '".mysqli_real_escape_string($group_name)."', `public` = '".(int)$public."'";

		}else{

			$sql = "UPDATE `"._DB_PREFIX."group` SET group_name = '".mysqli_real_escape_string($group_name)."', `public` = '".(int)$public."' WHERE group_id = '".mysqli_real_escape_string($group_id)."'";
	...
	public function delete_group($db,$group_id){
		...
		$sql = "DELETE FROM `"._DB_PREFIX."group` WHERE group_id = '".mysqli_real_escape_string($group_id)."' LIMIT 1";

		$res = query($sql,$db);

		$sql = "DELETE FROM `"._DB_PREFIX."member_group` WHERE group_id = '".mysqli_real_escape_string($group_id)."'";
	...
	public function get_image($db,$id){
		...
		$sql = "SELECT * FROM `"._DB_PREFIX."image` WHERE image_id = '".mysqli_real_escape_string($id)."'";
	...
	public function get_link($db,$id){
		...
		$sql = "SELECT * FROM `"._DB_PREFIX."link` WHERE link_id = '".mysqli_real_escape_string($id)."'";

		$link = array_shift(qa($sql,$db));

		// find open rates

		$sql = "SELECT * FROM `"._DB_PREFIX."link_open` WHERE link_id = '".mysqli_real_escape_string($id)."'";
	...
	public function record_open($db,$send_id,$member_id){
		...
		$sql = "UPDATE `"._DB_PREFIX."newsletter_member` SET open_time = '".time()."' WHERE send_id = '".mysqli_real_escape_string($send_id)."' AND member_id = '".mysqli_real_escape_string($member_id)."' LIMIT 1";
	...
	public function record_link_click($db,$send_id,$member_id,$link_id){
		...
		$sql = "INSERT INTO `"._DB_PREFIX."link_open` SET `timestamp` = '".time()."', link_id = '".(int)$link_id."', member_id = '".mysqli_real_escape_string($member_id)."', send_id ='".mysqli_real_escape_string($send_id)."'";
	...
	public function get_member($db,$member_id,$full=false){

		$member_id = 1; //(int)$member_id;

		if(!$db)$db = db_connect();

		$sql = "SELECT * FROM `"._DB_PREFIX."member` WHERE `member_id` = '".$member_id."'";
		...
		if($full)
		{
			...
			$sql = "SELECT * FROM `"._DB_PREFIX."member_group` WHERE `member_id` = '".mysqli_real_escape_string($member_id)."'";
			...
			$sql = "SELECT * FROM `"._DB_PREFIX."campaign_member` LEFT JOIN `"._DB_PREFIX."campaign` USING (campaign_id) WHERE `member_id` = '".mysqli_real_escape_string($member_id)."'";
			...
			$sql = "SELECT * FROM `"._DB_PREFIX."sync_member` LEFT JOIN `"._DB_PREFIX."sync` USING (sync_id) WHERE `member_id` = '".mysqli_real_escape_string($member_id)."'";
			...
			$sql = "SELECT *,nm.send_id AS id FROM `"._DB_PREFIX."newsletter_member` nm LEFT JOIN `"._DB_PREFIX."send` s USING (send_id) WHERE nm.`member_id` = '".mysqli_real_escape_string($member_id)."'";

			$member['sent'] = qa($sql,$db);

			$sql = "SELECT *,nm.send_id AS id FROM `"._DB_PREFIX."newsletter_member` nm LEFT JOIN `"._DB_PREFIX."send` s USING (send_id) WHERE nm.`member_id` = '".mysqli_real_escape_string($member_id)."' AND open_time > 0";

			$member['opened'] = qa($sql,$db);

			$sql = "SELECT *,nm.send_id AS id FROM `"._DB_PREFIX."newsletter_member` nm LEFT JOIN `"._DB_PREFIX."send` s USING (send_id) WHERE nm.`member_id` = '".mysqli_real_escape_string($member_id)."' AND bounce_time > 0";

			$member['bounces'] = qa($sql,$db);

			$sql = "SELECT *,s.send_id AS id FROM `"._DB_PREFIX."member` m LEFT JOIN `"._DB_PREFIX."send` s ON m.unsubscribe_send_id = s.send_id WHERE m.`member_id` = '".mysqli_real_escape_string($member_id)."' AND m.unsubscribe_send_id != 0";
	...
	public function get_members($db,$group_id=false,$newest_first=false,$limit=false,$search=array()){
		...
		if(isset($search['name']) && $search['name']){
			...
			$sql .= " AND (m.first_name LIKE '%".mysqli_real_escape_string($search['name'])."%' OR m.last_name LIKE '%".mysqli_real_escape_string($search['name'])."%')";

		}

		if(isset($search['email']) && $search['email']){

			$sql .= " AND m.email LIKE '%".mysqli_real_escape_string($search['email'])."%' ";
		...
		if(isset($search['start-letter']) && $search['start-letter']){
			...
			if($search['start-letter'] == '#'){
				...
				$sql .= " AND m.email LIKE '".mysqli_real_escape_string($search['start-letter'])."%' ";
	...
	public function save_member($db=false,$member_id='new',$fields,$from_public_sub=false){
		...
		if($fields['email']){
			...
			$sql = "SELECT * FROM `"._DB_PREFIX."member` WHERE email = '".mysqli_real_escape_string($fields['email'])."'";
		...
		if($member_id == 'new'){
			...
			$where = " WHERE member_id = '".mysqli_real_escape_string($member_id)."' LIMIT 1";
		...
		if($member_id && is_array($groups)){
			...
			$sql = "DELETE FROM `"._DB_PREFIX."member_group` WHERE member_id = '".mysqli_real_escape_string($member_id)."'";
			...
			foreach($groups as $group_id){
				...
				$sql = "INSERT INTO `"._DB_PREFIX."member_group` SET member_id = '".mysqli_real_escape_string($member_id)."', group_id = '".mysqli_real_escape_string($group_id)."'";
	...
	public function save_member_custom($db,$member_id,$key,$val,$admin=false){
		...
		if(!$member_field_id && $admin){
			...
			$sql = "INSERT INTO "._DB_PREFIX."member_field SET field_name = '".mysqli_real_escape_string($key)."', field_type = 'text'";

			$res = query($sql,$db);

			$member_field_id = mysqli_insert_id($db);
		...
		if($member_field_id){
			...
			$sql = "REPLACE INTO "._DB_PREFIX."member_field_value SET member_id = '$member_id', member_field_id = '$member_field_id', value = '".mysqli_real_escape_string($val)."'";
	...
	public function delete_member($db,$member_id){
		...
		$sql = "DELETE FROM `"._DB_PREFIX."member` WHERE member_id = '".mysqli_real_escape_string($member_id)."' LIMIT 1";
	...
	public function unsubscribe($db,$member_id,$send_id=false){
		...
		$sql .= " WHERE member_id = '".mysqli_real_escape_string($member_id)."' LIMIT 1";
	...
	public function save_settings($db,$settings){
		...
		foreach($settings as $key=>$item){
			...
			$sql = "REPLACE INTO `"._DB_PREFIX."settings` SET `key` = '".mysqli_real_escape_string($newkey)."', `val` = '".mysqli_real_escape_string($newval)."'";
	...
	public function create_send($db,$newsletter_id,$send_groups,$dont_sent_duplicates,$send_later_date=false,$campaign_id=0){
			...
			foreach($send_groups as $group_id){
				...
				while($member = mysqli_fetch_assoc($members)){
					...
					if($dont_sent_duplicates){
						...
						$sql = "SELECT * FROM "._DB_PREFIX."newsletter_member nm LEFT JOIN `"._DB_PREFIX."send` s USING (send_id) WHERE nm.member_id = '".mysqli_real_escape_string($member['member_id'])."' AND s.newsletter_id = '".mysqli_real_escape_string($newsletter_id)."'";
		...
		if($send_members){
			...
			$sql = "INSERT INTO `"._DB_PREFIX."send` SET newsletter_id = '".mysqli_real_escape_string($newsletter_id)."', campaign_id = '".mysqli_real_escape_string($campaign_id)."', `status` = 1";
			...
			if($send_id){
				...
				if (is_file(_NEWSLETTERS_DIR."newsletter-".$newsletter_id.".html")){
					...
					$sql = "UPDATE `"._DB_PREFIX."send` SET template_html = '".mysqli_real_escape_string($newsletter_html)."' WHERE send_id = '$send_id' LIMIT 1";
				...
				if(is_file(_NEWSLETTERS_DIR."newsletter-".$newsletter_id."-full.html")){
					...
					$sql = "UPDATE `"._DB_PREFIX."send` SET full_html = '".mysqli_real_escape_string($newsletter_html)."' WHERE send_id = '$send_id' LIMIT 1";
	...
	public function get_send($db,$send_id){
		...
		$sql = "SELECT * FROM `"._DB_PREFIX."send` WHERE `send_id` = '".mysqli_real_escape_string($send_id)."'";

		$send = array_shift(qa($sql,$db));

		$sql = "SELECT * FROM `"._DB_PREFIX."newsletter_member` nm LEFT JOIN `"._DB_PREFIX."member` m USING (member_id) WHERE nm.send_id = '".mysqli_real_escape_string($send_id)."' AND nm.status = 1 AND m.unsubscribe_date = '0000-00-00'";

		$send['unsent_members'] = qa($sql,$db);

		$sql = "SELECT * FROM `"._DB_PREFIX."newsletter_member` nm LEFT JOIN `"._DB_PREFIX."member` m USING (member_id) WHERE nm.send_id = '".mysqli_real_escape_string($send_id)."' AND nm.status != 1";

		$send['sent_members'] = qa($sql,$db);

		$sql = "SELECT * FROM `"._DB_PREFIX."newsletter_member` nm LEFT JOIN `"._DB_PREFIX."member` m USING (member_id) WHERE nm.send_id = '".mysqli_real_escape_string($send_id)."' AND m.unsubscribe_date != '0000-00-00' AND m.unsubscribe_send_id = '".mysqli_real_escape_string($send_id)."'";

		$send['unsub_members'] = qa($sql,$db);

		$sql = "SELECT * FROM `"._DB_PREFIX."newsletter_member` nm LEFT JOIN `"._DB_PREFIX."member` m USING (member_id) WHERE nm.send_id = '".mysqli_real_escape_string($send_id)."' AND nm.open_time > 0";

		$send['opened_members'] = qa($sql,$db);

		$sql = "SELECT * FROM `"._DB_PREFIX."newsletter_member` nm LEFT JOIN `"._DB_PREFIX."member` m USING (member_id) WHERE nm.send_id = '".mysqli_real_escape_string($send_id)."' AND nm.bounce_time > 0";
	...
	public function pause_send($db,$send_id){
		...
		$sql = "UPDATE `"._DB_PREFIX."send` SET status = 6 WHERE `send_id` = '".mysqli_real_escape_string($send_id)."'";
	...
	public function un_pause_send($db,$send_id){
		...
		$sql = "UPDATE `"._DB_PREFIX."send` SET status = 1 WHERE `send_id` = '".mysqli_real_escape_string($send_id)."'";
	...
	public function send_complete($db,$send_id){
		...
		$sql = "UPDATE `"._DB_PREFIX."send` SET status = 3, finish_time = '".time()."' WHERE `send_id` = '".mysqli_real_escape_string($send_id)."'";
	...
	public function get_newsletter_sends($db,$newsletter_id){
		...
		$sql = "SELECT * FROM `"._DB_PREFIX."send` WHERE newsletter_id = '".mysqli_real_escape_string($newsletter_id)."' AND start_time <= '".time()."'";
	...
	public function send_out_newsletter($db,$send_id,$member_id,$newsletter_id=false,$force=false){
		...
		if($status){
			...
			$replace = array(
				...
				/*"unsubscribe_url" => 'https://'.$this->base_href.'/ext.php?t=unsub&sid='.$send_id.'&mid='.$member_data['member_id'].'&hash='.md5("Unsub ".$member_data['member_id']."from $send_id").'',

				"view_online"=>'https://'.$this->base_href.'/ext.php?t=view&id='.$send_data['newsletter_id'].'&sid='.$send_id.'&mid='.$member_data['member_id'].'&hash='.md5("view link ".$member_data['member_id']."from $send_id").'',
			...
			if($send_email_status){
				...
				$sql = "UPDATE "._DB_PREFIX."newsletter_member SET `status` = 3, sent_time = '".time()."' WHERE `member_id` = '".mysqli_real_escape_string($member_data['member_id'])."' AND send_id = '".$send_id."' LIMIT 1";
				...
				$sql = "UPDATE "._DB_PREFIX."newsletter_member SET `status` = 4, sent_time = '".time()."', bounce_time = '".time()."' WHERE `member_id` = '".mysqli_real_escape_string($member_data['member_id'])."' AND send_id = '".$send_id."' LIMIT 1";
	...
	public function version_url(){
		...
		$url = "https://tf.dtbaker.com.au/newsletter/version.php";
	...
	public function get_campaign($db=false,$campaign_id){
		...
		$sql = "SELECT * FROM "._DB_PREFIX."campaign WHERE campaign_id = '".$campaign_id."'";

		$campaign = array_shift(qa($sql,$db));

		 $sql = "SELECT * FROM "._DB_PREFIX."campaign_member cm LEFT JOIN "._DB_PREFIX."member m USING (member_id) WHERE campaign_id = '".$campaign_id."'";

		$campaign['members_rs'] = qa($sql,$db);

	 	$sql = "SELECT * FROM "._DB_PREFIX."campaign_newsletter cm LEFT JOIN "._DB_PREFIX."newsletter n USING (newsletter_id) WHERE campaign_id = '".$campaign_id."' ORDER BY send_time ASC";

		$campaign['newsletter_rs'] = qa($sql,$db);
	...
	public function save_campaign($db,$fields,$campaign_id){
		...
		if($campaign_id == 'new'){
			...
			$where = " WHERE campaign_id = '".mysqli_real_escape_string($campaign_id)."' LIMIT 1";
	...
	public function get_sync($db=false,$sync_id){
		...
		$sql = "SELECT * FROM `"._DB_PREFIX."sync` WHERE sync_id = '".mysqli_real_escape_string($sync_id)."'";

		$sync = array_shift(qa($sql,$db));

		$sql = "SELECT COUNT(sync_id) AS c FROM "._DB_PREFIX."sync_member WHERE sync_id = '".mysqli_real_escape_string($sync_id)."'";

		$res = array_shift(qa($sql,$db));

		$sync['member_count'] = $res['c'];

		$sql = "SELECT group_id AS id FROM "._DB_PREFIX."sync_group WHERE sync_id = '".mysqli_real_escape_string($sync_id)."'";
	...
	public function save_sync($db,$fields,$sync_id){
		...
		if($sync_id == 'new'){
			...
			$where = " WHERE sync_id = '".mysqli_real_escape_string($sync_id)."' LIMIT 1";
		...
		if($sync_id=='new'){
			...
			$sync_id = mysqli_insert_id($db);

		}

		if($sync_id && is_array($groups)){

			$sql = "DELETE FROM `"._DB_PREFIX."sync_group` WHERE sync_id = '".mysqli_real_escape_string($sync_id)."'";
			...
			foreach($groups as $group_id){
				...
				$sql = "INSERT INTO `"._DB_PREFIX."sync_group` SET sync_id = '".mysqli_real_escape_string($sync_id)."', group_id = '".mysqli_real_escape_string($group_id)."'";
	...
	public function test_sync($db,$sync_id){
		...
		$syncdbcnx = @mysqli_connect($db_host2,$db_user2,$db_pass2); // or die("Failed on '$db_host2' '$db_user2' '$db_pass2'".mysqli_error());

		if(!$syncdbcnx){

			echo 'Sync failed to connect to database. Please check username/password/host.' . mysqli_error($syncdbcnx);

			exit;

		}

		$syncdb = @mysqli_select_db($sync['db_name']);

		if(!$syncdb){

			ob_end_clean();

			echo 'Sync failed to select the database \''.$sync['db_name'].'\'. ' . mysqli_error();

			exit;

		}

		$sql = "SELECT `".mysqli_real_escape_string($sync['db_table_key'])."`, `".mysqli_real_escape_string($sync['db_table_email_key'])."` ";

		if($sync['db_table_fname_key']) $sql .= ", `".mysqli_real_escape_string($sync['db_table_fname_key'])."`";

		if($sync['db_table_lname_key']) $sql .= ", `".mysqli_real_escape_string($sync['db_table_lname_key'])."`";

		$sql .= " FROM `".mysqli_real_escape_string($sync['db_table'])."` LIMIT 1";

		$res = @mysqli_query($sql,$syncdbcnx);

		if(!$res){

			ob_end_clean();

			echo 'Failed to select members. Please ensure your table name, primary key and email key are correct. '.mysqli_errno();
	...
	public function run_sync($db,$sync_id){
		...
		if($syncdbcnx){
			...
			$sql .= " FROM `".mysqli_real_escape_string($sync['db_table'])."`";
				...
				if($sync_unique_id){
					...
					if($email_address){
						...
						if(!$member_id){
							...
							$sql = "SELECT * FROM `"._DB_PREFIX."member` WHERE `email` = '".mysqli_real_escape_string($email_address)."'";
						...
						if($member_id){
							...
							//`email` = '".mysqli_real_escape_string($email_address)."',

							$sql = "UPDATE `"._DB_PREFIX."member` SET `email` = '".mysqli_real_escape_string($email_address)."' ";

							if($sync['db_table_fname_key']) $sql .= ", first_name = '".mysqli_real_escape_string($row[$sync['db_table_fname_key']])."'";

							if($sync['db_table_lname_key']) $sql .= ", last_name = '".mysqli_real_escape_string($row[$sync['db_table_lname_key']])."'";
	...
	public function get_form($db,$include_form_tag = false,$member_id=false,$admin=false){
		...
		<form action="https://<?php echo $this->base_href;?>/ext.php?t=signup" method="post">

		<?php } ?>

		

		<table cellpadding="3" width="100%" class="table-bordered">
