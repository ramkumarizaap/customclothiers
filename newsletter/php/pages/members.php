...
<?php
	...
	$pagable_members = $newsletter->get_members($db,false,false,false,$search); // used to work out how many pages
	 ...
	 $page_count = ceil( count($pagable_members) / $per_page_limit) ;
// echo "hi";exit;
...
<div class="box">
	...
	<table cellpadding="5" class="search-members">
		...
		<tr>
			...
			<td valign="middle" class="text-center">
				 ...
				 <input type="submit" name="search" class="members-search" value="Search!">
...
<h2><span>Member List (<?php echo count($all_members);?>)</span></h2>
...
<div class="box">
	...
	<table cellpadding="5">
		...
		<thead>
			...
			<tr>
				...
				<td class="alphabet" colspan="<?php echo 11+count($member_fields);?>">
	...
	?>
		...
		if(count($pagable_members)>0&&count($pagable_members)>=$limit_start){
		...
		foreach ($pagable_members as $mem) {
			if($x++ < $per_page_limit)
			{

		// while( $x++ < $per_page_limit && $member = mysql_fetch_assoc($pagable_members)){

			$member = $newsletter->get_member($db,$mem['member_id']);
			...
			<tr>
				...
				<td>
					...
					<a href="?p=members&edit_member_id=<?php echo $member['member_id'];?>" class="btn btn-small"><i class="fa fa-pencil-square-o square"></i></a> 

					<a href="?p=members&delete_member_id=<?php echo $member['member_id'];?>" onclick="if(confirm('Really delete this member?'))return true;else return false;"class="btn btn-small"><i class="fa fa-trash-o trash"></i></a>
...
<div class="box">
	...
	<a href="?p=members&export=true" class="csv">Export all members to CSV</a>
