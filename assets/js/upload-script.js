$(document).ready(function() {
	...
	*****************************/	
	var getCanvasImage = function(image) {
		...
		var croping = "crop";//$('input[name=croping]:checked').val();
	...
	*****************************/	
	var createPreview = function(file, newURL) {	
		...
		.css({
			...
			'MozTransform': 'scale(1) rotate(0deg)',
			'webkitTransform': 'scale(1) rotate(0deg)',
			'OTransform': 'scale(1) rotate(0deg)',
			'z-index': zindex++
		})
		.show();
	...
	****************************/
	var uploadToServer = function(oldFile, newFile) {
		...
		$.ajax({
			url: 'https://customclothiers.com/cp/upload-carousel.php',
			type: 'POST',
			data: formData,
			processData: false,
			contentType: false,
